...
//
// Orchestrates engine-driven text-block detection and writes results into
// documentStore / engineStore. Extracted from ToolSidebar so the same logic
// can be triggered automatically when a PDF is opened.
...
/**
 * Run engine text-block detection for the current page.
 *
 * 如果当前页已有 text-block overlay(包括用户已编辑过的),直接跳过 ——
 * 重新检测会先 wipe 该页所有 text-block,把用户的编辑一并清掉。此时返回
 * true(视为已就绪),想重新检测可先删除该页的块。
 *
 ...
 */
export async function runEngineDetection(): Promise<boolean> {
